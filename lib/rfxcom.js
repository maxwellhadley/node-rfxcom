...
RfxCom.prototype.open = function() {
    ...
    if (typeof self.serialport === "undefined" || self.fakingSerialPort) {
        if (typeof self.serialport === "undefined") {
            // Delay opening the serial port until after the event handlers are installed
...
RfxCom.prototype.close = function () {
    ...
    if (self.serialport && self.serialport.fd) {
...
RfxCom.prototype.statusHandler = function(data) {
    ...
    // handle RFY list remotes status response
    if (subtype == 0x03) {
        var params = {
            remoteNumber: data[3],
            deviceId: self.dumpHex(data.slice(4, 8)).join(""),
            id: [data[4], data[5], data[6]],
            unitCode: data[7],
            unknown: data.slice(8)
        };

        self.emit("list", params);
    } else {
        var msg = data.slice(2, 13),
...
/**
 *
 * Called by the data event handler when data arrives from a device using the PT2262 chip
